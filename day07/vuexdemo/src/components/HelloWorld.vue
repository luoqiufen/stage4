<template>
  <div class="hello">
    <p>{{ $store.state.num }} | {{ showNum }} | {{ showNum2 }} | {{ num2 }} | {{ num }}</p>
    <p>{{ stus[0].name }} </p>
    <p>{{ getNames }}</p>
    <p>{{ getStu }}</p>
...
export default {
  ...
  computed: {
    ...
    // 方式三: 参数映射状态
    // 数组的元素对应着store中state里面定义的状态属性,必须保持一致,使用时,在模板中就是使用num
    // fanhuide {num2: state.num2}
    ...
    getStu(){
      return this.$store.getters.getStuById(1001);
    }
  },
  methods: {
    show(){
